# Attribution

This compilation is licensed under [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/).

Suggested attribution:

> Data from [CalcuGrid](https://calcugrid.com), CC BY 4.0

## What the licence does and does not cover

The **underlying facts** — a breed standard, a tax rate, a national grading
scale — are not copyrightable and you may use them freely regardless of this
licence. What the licence covers is this particular selection, arrangement and
verification of them.

In practice: if you take three numbers out of a CSV, you owe nothing. If you
redistribute the datasets, please credit them.

## Sources

| Dataset | Source |
|---|---|
| Dog breeds | American Kennel Club breed standards |
| Dog energy formula | Merck Veterinary Manual, small-animal nutritional requirements |
| State income tax | State revenue department publications, TY2026 |
| Grading scales | National qualifications frameworks and ministry of education grading regulations |
| US GPA convention | The mapping common to US credential evaluation — not an official conversion |
| GPU specs | Manufacturer specification pages (NVIDIA, AMD) |
| EV efficiency | US EPA, fueleconomy.gov |
| Electricity rates | US EIA Electric Power Monthly, residential average |

Where a source publishes its own terms, those terms govern the underlying facts
independently of this compilation.
